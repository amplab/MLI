...
object NGrams extends FeatureExtractor with Serializable {
   ...
   */
  def buildDictionary(table: MLTable, numGrams: Int, numFeatures: Int): Seq[(String,Int)] = {

    //This is the classic Word Count, sorted descending by frequency.
    val freqTable = table.flatMap(rowToNgramRows(_, numGrams))
  ...
  def ngrams(str: String, n: Int, sep: String="_"): Set[String] = {
    //Lowercase and split on whitespace.
    val toks = str.toLowerCase.replaceAll(sep,"").split("\\s+").map(normalize)
  ...
  def rowToNgramRows(row: MLRow, n: Int): Seq[MLRow] = {
    val grams = ngrams(row(0).toString, n)
  ...
  def rowToFilteredNgrams(row: MLRow, dict: Seq[(String,Int)], n: Int): MLRow = {

    //Pull out the ngrams for a specific string.
    val x = ngrams(row(0).toString, n)
  ...
  def extractNGrams(in: MLTable, n: Int=1, k: Int=20000): MLTable = {
    ...
    val out = in.map(rowToFilteredNgrams(_, dict, n))
